...
class LivenessCheckProcessor_min_no_networking: NSObject, FaceTecFaceScanProcessorDelegate {
    init(fromViewController: UIViewController, sessionToken: String) {
        super.init()
        
        // Core FaceTec Device SDK code that starts the User Session.
        let livenessCheckSessionViewController = FaceTec.sdk.createSessionVC(faceScanProcessorDelegate: self, sessionToken: sessionToken)
        fromViewController.present(livenessCheckSessionViewController, animated: true, completion: nil)
    }
